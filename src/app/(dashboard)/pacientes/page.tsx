...
import {
  ...
  Phone, Clock, X, ShieldAlert, Calendar, Pencil,
} from 'lucide-react'
...
function SortButton({ label, column, current, order, onClick }: {
  label: string; column: SortBy; current: SortBy; order: SortOrder; onClick: (col: SortBy) => void
}) {
  const active = current === column
  return (
    <button className="flex items-center gap-1 font-medium text-foreground-muted hover:text-foreground group" onClick={() => onClick(column)}>
      {label}
      {active
        ? order === 'asc' ? <ArrowUp className="h-3.5 w-3.5 text-primary" /> : <ArrowDown className="h-3.5 w-3.5 text-primary" />
...
export default function PacientesPage() {
  ...
  const { data, isLoading, error } = useQuery<PatientsResponse>({
    ...
    queryFn: async () => {
      const params = new URLSearchParams({ search: debouncedSearch, page: String(page), limit: String(limit), sortBy, sortOrder })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-semibold tracking-tight text-foreground">Pacientes</h1>
          {data && (
            <p className="mt-0.5 text-sm text-foreground-muted">{data.total} pacientes activos</p>
          )}
        </div>
          ...
          <div className="p-12 text-center">
            ...
            {!search && (
              <Button asChild className="mt-4">
                <Link href="/pacientes/nuevo"><Plus className="mr-2 h-4 w-4" />Nuevo Paciente</Link>
              </Button>
            )}
          </div>
        ) : (
          <>
              ...
              {data?.patients.map((patient) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center gap-3 px-4 py-3 hover:bg-surface-alt/70 active:bg-border-subtle transition-colors"
                  >
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius)] bg-[var(--avatar-bg)] text-sm font-semibold text-white">
                      {patient.firstName[0]}{patient.lastName[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <p className="font-medium text-foreground truncate">{patient.firstName} {patient.lastName}</p>
                        {patient.allergies.length > 0 && <ShieldAlert className="h-3.5 w-3.5 text-danger shrink-0" />}
                      </div>
                      <p className="text-xs text-foreground-subtle truncate">
                        {documentTypeLabel(patient.documentType)}: {patient.documentNumber} · {calculateAge(patient.birthDate)} años
                      </p>
                      <div className="mt-0.5 flex items-center gap-1.5">
                        <LastVisitDot date={patient.clinicalRecords[0]?.date} now={now} />
                        <span className="text-xs text-foreground-muted">
                          {patient.clinicalRecords[0] ? formatDate(patient.clinicalRecords[0].date) : 'Sin visitas'}
                          {patient._count.clinicalRecords > 0 && (
                            <span className="ml-1 text-foreground-subtle">({patient._count.clinicalRecords})</span>
                          )}
                        </span>
                        {nextAppt && (
                          <span className="inline-flex items-center gap-1 rounded-full border border-border bg-surface-alt px-2 py-0.5 text-xs font-medium text-foreground-muted">
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-surface-alt/85">
                    <th className="px-4 py-2.5 text-left">
                      <SortButton label="Paciente" column="name" current={sortBy} order={sortOrder} onClick={handleSort} />
                    </th>
                    <th className="px-4 py-2.5 text-left font-medium text-foreground-muted">Documento</th>
                    <th className="px-4 py-2.5 text-left">
                      <SortButton label="Edad" column="age" current={sortBy} order={sortOrder} onClick={handleSort} />
                    </th>
                    <th className="px-4 py-2.5 text-left font-medium text-foreground-muted hidden lg:table-cell">Teléfono</th>
                    <th className="px-4 py-2.5 text-left">
                      <SortButton label="Última Visita" column="lastVisit" current={sortBy} order={sortOrder} onClick={handleSort} />
                    </th>
                    <th className="px-4 py-2.5 text-left font-medium text-foreground-muted">Próxima Cita</th>
                      ...
                      >
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-3">
                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius)] bg-[var(--avatar-bg)] text-xs font-semibold text-white">
                              {patient.firstName[0]}{patient.lastName[0]}
                            </div>
                            <div className="flex items-center gap-1.5">
                        ...
                        <td className="px-4 py-2.5 text-foreground-muted">{calculateAge(patient.birthDate)} años</td>
                        ...
                        <td className="px-4 py-2.5">
                          {nextAppt ? (
                            <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface-alt px-2.5 py-1 text-xs font-medium text-foreground-muted">
                              <Clock className="h-3 w-3" />{formatDate(nextAppt.dateTime)}
                            </span>
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <Button variant="ghost" size="sm" className="h-8 px-2 text-xs text-foreground-muted hover:text-foreground" asChild>
                              <Link href={`/pacientes/${patient.id}/editar`} onClick={e => e.stopPropagation()}>
