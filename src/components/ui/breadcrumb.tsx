...
export function Breadcrumb({ items, className }: BreadcrumbProps) {
  return (
    <nav className={cn('hidden sm:flex items-center gap-1 text-sm text-foreground-muted min-w-0 overflow-hidden', className)}>
      {items.map((item, index) => {
        const isLast = index === items.length - 1
        return (
          <span key={index} className="flex items-center gap-1 min-w-0 shrink-0 last:shrink">
            {index > 0 && <ChevronRight className="h-3.5 w-3.5 text-border shrink-0" />}
            {item.href && !isLast ? (
